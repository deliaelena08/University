...
int main1111(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        while (total_citit < N && id_curent < size) {
            ...
            MPI_Send(&to_read, 1, MPI_INT, id_curent, TAG_LEN, MPI_COMM_WORLD);
            MPI_Send(buf1.data(), to_read, MPI_INT, id_curent, TAG_A, MPI_COMM_WORLD);
            MPI_Send(buf2.data(), to_read, MPI_INT, id_curent, TAG_B, MPI_COMM_WORLD);
