...
def menu_1():
    print(
        "1.1 Adaugă o nouă cheltuială (se specifică ziua, suma, tipul) \n1.2 Actualizează cheltuială (se specifică ziua, suma, tipul) \n1.3 Afisare dictionar ")


def menu_2():
    print("2.1 Șterge toate cheltuielile pentru ziua data")
    print(
        "2.2 Șterge cheltuielile pentru un interval de timp (se dă ziua de început și sfârșit, se șterg toate cheltuielile pentru perioada dată) ")
...
def validare_cheltuieli(cheltuiala):
    ziua=int(cheltuiala['ziua'])
    errors=[]
    if ziua<1 or ziua>31 :
        errors.append("Ziua calendaristica nu este introdusa corect")
    if cheltuiala['tip'] not in ['telefon','mancare','intretinere','imbracaminte','altele']:
        errors.append("Tip de cheltuiala introdusa gresit")
    if cheltuiala['suma']<0:
        errors.append("Suma nu este introdusa corect.Este nevoie de un numar natural")
    if len(errors)>0:
        error_string='\n'.join(errors)
        raise ValueError(error_string)

def verificarea_validarii(cheltuaiala):
    try:
        validare_cheltuieli(cheltuaiala)
        assert True
    except ValueError:
        assert False
...
def test1():
    ...
    undo(undo_list, cheltuiala_list)
    assert (len(cheltuiala_list) == 3)
    assert(stergere_cheltuiala_ziua('20',cheltuiala_list,undo_list))
    assert(len(cheltuiala_list)==2)
    undo(undo_list, cheltuiala_list)
    assert(stergere_cheltuieli_suma(6,cheltuiala_list,undo_list))
    print(cheltuiala_list)
    assert (len(cheltuiala_list) == 2)
    undo(undo_list, cheltuiala_list)
    assert(cheltuieli_din_urma(15,cheltuiala_list)==[{'ziua': "10", 'suma': 5, 'tip': 'telefon'}])
    '''
        ...
        case 1:
            ...
            match functionalitate:
                case 1.1:
                    ...
                    #verificarea_validarii(cheltuiala)
                ...
                case 1.2:
                    ...
                    print("Suma de", suma_actualizata, "pentru", tipul_sumei,
                          "a fost actualizata ultima data pentru data de", data_actualizarii)
                case 1.3:
                    print(cheltuiala_list)
        case 2:
            menu_2()
